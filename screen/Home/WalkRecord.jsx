...
export default function WalkRecord() {
  ...
  const hours = Array.from({ length: 24 }, (v, k) => ({
    label: String(k).padStart(2, "0"),
    value: k,
  }));
  const minutes = Array.from({ length: 60 }, (v, k) => ({
    label: String(k).padStart(2, "0"),
    value: k,
  }));
  const seconds = Array.from({ length: 60 }, (v, k) => ({
    label: String(k).padStart(2, "0"),
    value: k,
  }));
  ...
  const handleComplete = async () => {
    ...
    try {
      const response = await axios.post(
        `http://${Uip}:5001/add_walking_event`,
        {
