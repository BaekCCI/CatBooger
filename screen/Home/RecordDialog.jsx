  ...
  const handleComplete = async () => {
    try {
      const response = await axios.post(
        `http://${Uip}:5001/add_feeding_event`,
        {
...
const Medicine = ({ info }) => {
  ...
  const handleComplete = async () => {
    try {
      const response = await axios.post(
        `http://${Uip}:5001/add_medication_event`,
        {
          userId: String(userId), // 실제 사용자 ID로 대체
...
const Poop = ({ info }) => {
  ...
  const handleComplete = async () => {
    try {
      const response = await axios.post(
        `http://${Uip}:5001/add_defecation_event`,
        {
        ...
        <View>
          {!poopBtn ? (
            <TextBtn onPress={handlePoopBtn}>
              <TypeText size="18px" weight="semi-bold" color="#139989">
                대변 추가 +
              </TypeText>
            </TextBtn>
          ) : (
            <View>
              ...
              <RowView>
                <ColorBtn
                  selected={poopColor === "none"}
                  onPress={() => handlePoopColor("none")}
                >
                  <XLine />
                </ColorBtn>
                <ColorBtn
                  color="brown"
                  selected={poopColor === "brown"}
                  onPress={() => handlePoopColor("brown")}
                ></ColorBtn>
                <ColorBtn
                  color="#FFC700"
                  selected={poopColor === "yellow"}
                  onPress={() => handlePoopColor("yellow")}
                ></ColorBtn>
                <ColorBtn
                  color="black"
                  selected={poopColor === "black"}
                  onPress={() => handlePoopColor("black")}
                ></ColorBtn>
                <ColorBtn
                  color="green"
                  selected={poopColor === "green"}
                  onPress={() => handlePoopColor("green")}
                ></ColorBtn>
                <ColorBtn
                  color="red"
                  selected={poopColor === "red"}
                  onPress={() => handlePoopColor("red")}
                ></ColorBtn>
                <ColorBtn
                  color="gray"
                  selected={poopColor === "gray"}
                  onPress={() => handlePoopColor("gray")}
                ></ColorBtn>
              </RowView>
              ...
              <RowView>
                <StatusBtnWrap>
                  <StatusBtn
                    selected={poopStatus === "none"}
                    onPress={() => handlePoopStatus("none")}
                  >
                    <XLine />
                  </StatusBtn>
                ...
                <StatusBtnWrap>
                  <StatusBtn
                    selected={poopStatus === "원통형"}
                    onPress={() => handlePoopStatus("원통형")}
                  >
                    <StatusImg
                      source={require("../../assets/Home/Poop1.png")}
                    />
                  </StatusBtn>
                ...
                <StatusBtnWrap>
                  <StatusBtn
                    selected={poopStatus === "토끼똥"}
                    onPress={() => handlePoopStatus("토끼똥")}
                  >
                    <StatusImg
                      source={require("../../assets/Home/Poop2.png")}
                    />
                  </StatusBtn>
                ...
                <StatusBtnWrap>
                  <StatusBtn
                    selected={poopStatus === "무른똥"}
                    onPress={() => handlePoopStatus("무른똥")}
                  >
                    <StatusImg
                      source={require("../../assets/Home/Poop3.png")}
                    />
                  </StatusBtn>
                ...
                <StatusBtnWrap>
                  <StatusBtn
                    selected={poopStatus === "점액질"}
                    onPress={() => handlePoopStatus("점액질")}
                  >
                    <StatusImg
                      source={require("../../assets/Home/Poop4.png")}
                    />
                  </StatusBtn>
          ...
          {!peeBtn ? (
            ...
            <View>
              ...
              <RowView>
                <ColorBtn
                  selected={peeColor === "none"}
                  onPress={() => handlePeeColor("none")}
                >
                  <XLine />
                </ColorBtn>
                <ColorBtn
                  color="yellow"
                  selected={peeColor === "lightYellow"}
                  onPress={() => handlePeeColor("lightYellow")}
                ></ColorBtn>
                <ColorBtn
                  color="#FFCA0F"
                  selected={peeColor === "yellow"}
                  onPress={() => handlePeeColor("yellow")}
                ></ColorBtn>
                <ColorBtn
                  color="#FF9900"
                  selected={peeColor === "orange"}
                  onPress={() => handlePeeColor("orange")}
                ></ColorBtn>
                <ColorBtn
                  color="red"
                  selected={peeColor === "red"}
                  onPress={() => handlePeeColor("red")}
                ></ColorBtn>
                <ColorBtn
                  color="#FF9900"
                  selected={peeColor === "black"}
                  onPress={() => handlePeeColor("black")}
                >
                  <ColorImg
                    source={require("../../assets/Home/BrBkColor.png")}
                  />
                </ColorBtn>
                <ColorBtn
                  color="white"
                  selected={peeColor === "white"}
                  onPress={() => handlePeeColor("white")}
                >
                  <ColorImg
                    source={require("../../assets/Home/WhiteColor.png")}
                  />
                </ColorBtn>
              </RowView>
            </View>
...
const Vaccine = ({ info }) => {
  ...
  const handleComplete = async () => {
    try {
      const response = await axios.post(
        `http://${Uip}:5001/add_vaccination_event`,
        {
          userId: String(userId), // 실제 사용자 ID로 대체
      ...
      console.error(
        "Error adding vaccination event:",
        error.response ? error.response.data : error.message
      );
...
const ColorBtn = styled.TouchableOpacity`
  ...
  height: 30px;
  border: ${({ selected }) =>
    selected ? "3px solid #139989" : "1px solid #B3B3B3"};
  border-radius: 50px;
