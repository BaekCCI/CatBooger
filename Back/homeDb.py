...
def add_bathing_event():
    ...
    if not all([user_id, bathing_id, date]):
...
def add_defecation_event():
    ...
    if not all([user_id, defecation_id, date, state, type_]): #null값은 여기서 빼기
        return jsonify({"error": "Missing data fields"}), 400
...
def add_feeding_event():
    ...
    if not all([user_id, feeding_id, date]):
        return jsonify({"error": "Missing data fields"}), 400
...
def add_medication_event():
    ...
    if not all([user_id, medication_id, date, name]):
        return jsonify({"error": "Missing data fields"}), 400
...
def add_weight_event():
    ...
    if not all([user_id, weight_kg_id, date, weight_kg]):
        return jsonify({"error": "Missing data fields"}), 400
